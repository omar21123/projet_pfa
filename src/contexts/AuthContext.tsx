...
import { env } from "@/config/env"; // Importation de votre config pour l'URL API
import type {
  LaravelAuthResponse,
  LoginRequest,
...
interface AuthContextType {
  ...
  loginWithGoogle: (idToken: string, role: string, storeName?: string) => Promise<LaravelAuthResponse>;
}
...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const googleLoginMutation = useMutation<
    LaravelAuthResponse,
    Error,
    { idToken: string; role: string; storeName?: string }
  >({
    mutationFn: async ({ idToken, role, storeName }) => {
      const response = await axios.post(`${env.apiUrl}/api/auth/web/google`, {
        id_token: idToken,
        role,
    ...
    onSuccess: (response) => {
      ...
      const responseRole = (response as any).role as string | string[] | undefined;
      const resolvedRoles = responseRole
        ? Array.isArray(responseRole) ? responseRole : [responseRole]
        : extractRolesFromToken(token);

      // Extraction de l'email depuis le nouveau token si disponible
      let userEmail = "google-user@marche.ma"; 
      try {
  ...
  const loginWithGoogle = useCallback(
    async (idToken: string, role: string, storeName?: string): Promise<LaravelAuthResponse> => {
      return await googleLoginMutation.mutateAsync({ idToken, role, storeName });
    },
    [googleLoginMutation],
  );
